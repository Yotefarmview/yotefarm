...
const AdvancedMapComponent: React.FC<AdvancedMapComponentProps> = ({
  ...
  const createMeasureTooltip = useCallback(() => {
    ...
    if (mapInstance.current) {
      mapInstance.current.addOverlay(measureTooltip.current);
    }
  }, []);
  ...
  const handleSaveMeasurement = useCallback(() => {
    if (!editingMeasurement || !measurementSource.current) return;

    // Find the feature with this measurement
    ...
    if (feature) {
      ...
      setMeasurements(prev => 
        prev.map(m => m.id === editingMeasurement.id ? updatedMeasurement : m)
      );
    }

    setEditingMeasurement(null);
    setMeasurementForm({ name: '', isDrain: false });
  }, [editingMeasurement, measurementForm]);
    ...
    <div className="absolute top-4 right-4 z-50">
      <Card className="w-80 bg-white shadow-lg border">
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            <Label htmlFor="block-color">Cor do Bloco</Label>
            <UISelect value={editForm.color} onValueChange={(color) => setEditForm({...editForm, color})}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
          ...
          <div>
            <Label>Transparência: {Math.round((1 - editForm.transparency) * 100)}%</Label>
            <Slider
          ...
          <div className="p-3 bg-green-50 rounded-lg border border-green-200">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div>
                ...
                <p className="font-medium">{editingBlock.area_acres?.toFixed(2)} acres</p>
              </div>
              <div>
                <span className="text-green-700">Perímetro:</span>
                <p className="font-medium">{editingBlock.perimeter?.toFixed(0)}m</p>
              </div>
            </div>
          </div>
          ...
          <div className="text-xs text-gray-500 pt-2">
            <strong>Dica:</strong> Clique em qualquer bloco no mapa para editar rapidamente.
          </div>
        </CardContent>
      </Card>
    </div>
  ...
  const multiEditPanel = selectedFeatures.length > 0 && (
    <div className="absolute top-4 left-4 z-50">
      <Card className="w-80 bg-white shadow-lg border">
        ...
        <CardContent className="space-y-4">
          <div>
            <Label htmlFor="multi-color">Cor dos Blocos</Label>
            <UISelect value={multiEditForm.color} onValueChange={(color) => setMultiEditForm({...multiEditForm, color})}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent className="bg-white border shadow-lg z-50">
          ...
          <div>
            <Label>Transparência: {Math.round((1 - multiEditForm.transparency) * 100)}%</Label>
            <Slider
          ...
          <div className="text-xs text-gray-500 pt-2">
            <strong>Dica:</strong> Mantenha Shift pressionado + clique para selecionar múltiplos blocos.
          </div>
        </CardContent>
      </Card>
    </div>
